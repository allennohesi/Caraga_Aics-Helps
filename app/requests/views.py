...
def get_client_info(request, pk):
	...
	# Determine if the client has a transaction
	has_transaction = Transaction.objects.filter(client_id=data.id).exists()
	new = "2" if has_transaction else "1"
	...
	try:
		if request.method == "POST":
			with transaction.atomic():
				...
				# Check if there are any matching office stations
				if matching_office_stations.exists():
...
def printPagPamatuod(request, pk): #PettyCashVoucher
	...
	context = {
		'data': transaction,
		'service_provider': ServiceProvider.objects.filter(category="PHARMACY",status=1)[:14],
		'data': transaction,
		'categoryMedical': TypeOfAssistance.objects.filter(type_assistance_id=1,status=1),
		'provided_data': display_provided_data,
		'display_provider': display_provider,
